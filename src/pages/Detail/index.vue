<template>
  <div class="detail">
    ...
    <!-- 主要内容区域 -->
    <section class="con" v-if="goodsInfo.data">
      <!-- 导航路径区域 -->
      <div class="conPoin">
            ...
            <div class="chooseArea">
              ...
              <dl v-for="saleAttr in spuSaleAttrList" :key="saleAttr.id">
                ...
                <dd
                  ...
                  @click="chooseLight(saleAttr.spuSaleAttrValueList, value)"
...
export default {
  ...
  methods: {
    ...
    async addShopCar() {
      //发请求，将产品加入到数据库（通知服务器）
      //当前这里派发一个action，也想服务器发请求，判断加入购物车成功还是失败
      //成功进行路由跳转，失败对用户进行提示
      try {
        ...
        //简单的数据通过query形式给路由组件带过去
        //产品信息数据比较复杂的，通过会话存储（不持久化，会话结束数据才消失）
        //本地存储|会话存储 一般存储的是字符串
