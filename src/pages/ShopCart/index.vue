...
export default {
  ...
  methods: {
    ...
    //修改数量
    handler: throttle(async function (type, changed, cart) {
      switch (type) {
        case "mins":
          //判断产品的个数大于1才可以传递给服务器-1
          //如果小于1，传递给服务器0
          changed = cart.skuNum > 1 ? -1 : 0;
          break;
        case "change":
          if (isNaN(changed) || changed < 1) {
            // console.log(changed<1);
            changed = 0;
          } else {
            changed = parseInt(changed) - cart.skuNum;
          }
          break;
        case "plus":
          changed = 1;
    ...
    async deleteChecked() {
      try {
        await this.$store.dispatch("shopcart/deleteChecked");
        this.getData()
      } catch (error) {
        alert(error.massage);
      }
